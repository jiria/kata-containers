...
#
# Defaults to openvmm-snp: it is the attested stack the suite and the demo are
# run against. The previous default (qemu-coco-dev) made an omitted
# E2E_PLATFORM fail late and misleadingly — the run would get as far as looking
# for a kata-containers.img that the SNP paths never build, and report a missing
