...
pub enum FragmentError {
    ...
    RevokedCertificate,
    /// FR-1d: a certificate in the chain was not valid at the leaf's `notAfter`, i.e. the
    /// chain's own validity windows are mutually inconsistent. This is *not* a wall-clock
    /// expiry check — the guest has no trusted time source, so `now` is never consulted
    /// (see `did_x509::check_validity`).
    CertExpired,
